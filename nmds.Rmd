...
One point we unfortunately could not discuss in class (because of time `r emo::ji("sad")`), is the fact that like many other statistical tests, Permanova also has some assumptions about our data.
...
```{r, echo = FALSE}
ggdraw(tibble(distances = distances_betadispersion$distances,
       habitat_type = distances_betadispersion$group) %>%
  ggplot(aes(x = habitat_type, y = distances, fill = habitat_type))+
  geom_boxplot()) + 
  draw_label("tutorial version", color = "#FF000055", size = 60, angle = 45)
